...
def detect_license_plate(image_path, debug=True):
    ...
    blue_mask = cv2.inRange(hsv, color_ranges[0][0], color_ranges[0][1])
    green_mask = cv2.inRange(hsv, color_ranges[1][0], color_ranges[1][1])
    ...
    for contour in contours:
        ...
        contour_mask = np.zeros_like(plate_mask)
        cv2.drawContours(contour_mask, [contour], -1, 255, -1)
        
        # Check which color mask has more overlap with this contour
        blue_overlap = cv2.bitwise_and(blue_mask, contour_mask)
        green_overlap = cv2.bitwise_and(green_mask, contour_mask)
        blue_score = np.sum(blue_overlap)
        green_score = np.sum(green_overlap)
        
        # Only proceed if there's significant color detection
        if max(blue_score, green_score) < area * 50:  # Threshold can be adjusted
            continue
            
        # Rest of your existing contour filtering criteria
        if width < height:
        ...
        # Add to appropriate list based on color
        if blue_score > green_score:
            blue_candidates.append((contour, blue_score))
        else:
            green_candidates.append((contour, green_score))

    # Choose the best candidate based on color scores
    best_candidate = None
    if green_candidates:
        best_candidate = max(green_candidates, key=lambda x: x[1])[0]
    elif blue_candidates:
        best_candidate = max(blue_candidates, key=lambda x: x[1])[0]

    candidates = [best_candidate] if best_candidate is not None else []
    ...
    if debug and candidates:
        ...
        debug_images = [
            img,
            cv2.bitwise_and(img, img, mask=blue_mask),  # Show blue mask
            cv2.bitwise_and(img, img, mask=green_mask), # Show green mask
            debug_img,
            cropped_plate if cropped_plate is not None else np.zeros_like(img)
        ]
        debug_titles = ['Original', 'Blue Mask', 'Green Mask', 'Detected Plates', 'Cropped Plate']
        display_images(debug_images, debug_titles)
